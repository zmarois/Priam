...
public class DoubleRing
{
    ...
    public void doubleSlots()
    {
        ...
        for (PriamInstance data : local)
        {
            ...
            factory.create(data.getApp(), hash + slot, data.getInstanceId(), data.getHostName(), data.getHostIP(),
                    data.getRac(), data.getVolumes(), data.getToken());
        }
        ...
        for (PriamInstance data : filteredRemote(factory.getAllIds(config.getAppName())))
        {
            ...
            factory.create(data.getApp(), new_slot + hash, InstanceIdentity.DUMMY_INSTANCE_ID, config.getHostname(),
                    config.getHostIP(), data.getRac(), null, token);
        }
    }
    ...
    public void restore() throws IOException, ClassNotFoundException
    {
        ...
        try
        {
            ...
            for (PriamInstance data : allInstances)
                factory.create(data.getApp(), data.getId(), data.getInstanceId(), data.getHostName(), data.getHostIP(),
                        data.getRac(), data.getVolumes(), data.getToken());
