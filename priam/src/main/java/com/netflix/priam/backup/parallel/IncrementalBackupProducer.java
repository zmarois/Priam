...
public class IncrementalBackupProducer extends AbstractBackup implements IIncrementalBackup
{
    ...
    @Inject
    public IncrementalBackupProducer(IConfiguration config, Provider<AbstractBackupPath> pathFactory,
    ...
    {
        backupRestoreUtil = new BackupRestoreUtil(config.getIncrementalKeyspaceFilters(),
                config.getIncrementalCFFilter());
        //"this" is a producer, lets wake up the "consumers"
        this.incrementalConsumerMgr = new IncrementalConsumerMgr(this.taskQueueMgr,
                backupFileSystemCtx.getFileStrategy(config), super.config);
    ...
    protected void backupUploadFlow(File backupDir) throws Exception
    {
        for (final File file : backupDir.listFiles())
        {
            try
            {
                ...
                this.taskQueueMgr
                        .add(bp); //producer -- populate the queue of files.  *Note: producer will block if queue is full.
            }
            catch (Exception e)
            {
                logger.warn(
                        "Unable to queue incremental file, treating as non-fatal and moving on to next.  Msg: {} Fail to queue file: {}",
