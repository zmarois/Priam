...
public class S3EncryptedFileSystem extends S3FileSystemBase implements IBackupFileSystem, S3EncryptedFileSystemMBean
{
    ...
    public void download(AbstractBackupPath path, OutputStream os) throws BackupRestoreException
    {
        ...
        catch (Exception e)
        {
            throw new BackupRestoreException(
                    "Exception encountered downloading " + path.getRemotePath() + " from S3 bucket " + getPrefix(
                            this.config)
                            + ", Msg: " + e.getMessage(), e);
        }
    }
    ...
    public void upload(AbstractBackupPath path, InputStream in) throws BackupRestoreException
    {
        ...
        if (path.getSize() > 0)
            chunkSize = (path.getSize() / chunkSize >= MAX_CHUNKS) ?
                    (path.getSize() / (MAX_CHUNKS - 1)) :
                    chunkSize; //compute the size of each block we will upload to endpoint
        ...
        logger.debug(
                String.format("Uploading to %s/%s with chunk size %d", config.getBackupPrefix(), path.getRemotePath(),
                        chunkSize));
