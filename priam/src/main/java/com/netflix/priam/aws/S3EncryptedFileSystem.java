...
public class S3EncryptedFileSystem extends S3FileSystemBase implements S3EncryptedFileSystemMBean
{
    ...
    @Inject
    public S3EncryptedFileSystem(Provider<AbstractBackupPath> pathProvider, ICompression compress,
    ...
    )
    {
        ...
        super.s3Client = AmazonS3Client.builder().withCredentials(cred.getAwsCredentialProvider())
                .withRegion(config.getDC()).build();
    }
    ...
    public void downloadFile(AbstractBackupPath path, OutputStream os) throws BackupRestoreException
    {
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new BackupRestoreException(
                        "Exception encountered when copying bytes from input to output during download", ex);
        ...
        {
            throw new BackupRestoreException(
                    "Exception encountered downloading " + path.getRemotePath() + " from S3 bucket " + getPrefix(config)
                            + ", Msg: " + e.getMessage(), e);
        }
    }
    ...
    public void uploadFile(AbstractBackupPath path, InputStream in, long chunkSize) throws BackupRestoreException
    {
        ...
        InitiateMultipartUploadRequest initRequest = new InitiateMultipartUploadRequest(config.getBackupPrefix(),
                path.getRemotePath()); //initialize chunking request to aws
        InitiateMultipartUploadResult initResponse = s3Client
                .initiateMultipartUpload(initRequest); //Fetch the aws generated upload id for this chunking request
        ...
        catch (IOException e)
        {
            String message = String
                    .format("Exception in compressing the input data during upload to EncryptedStore  Msg: " + e
                            .getMessage());
        ...
        try
        {
            ...
            {
                throw new BackupRestoreException(
                        "Number of parts(" + partNum + ")  does not match the expected number of uploaded parts("
                                + partETags.size() + ")");
            }

            CompleteMultipartUploadResult resultS3MultiPartUploadComplete = new S3PartUploader(s3Client, part,
                    partETags)
                    .completeUpload(); //complete the aws chunking upload by providing to aws the ETag that uniquely identifies the combined object data
