...
public class SSTableLoaderWrapper
{
    ...
    private static Set<Component> allComponents = Sets
            .newHashSet(Component.COMPRESSION_INFO, Component.DATA, Component.FILTER, Component.PRIMARY_INDEX,
                    Component.STATS, Component.DIGEST);
    ...
    public Collection<PendingFile> stream(File directory) throws IOException, InterruptedException
    {
        ...
        for (SSTableReader sstable : loader.openSSTables())
        {
            ...
            PendingFile pending = new PendingFile(sstable, desc, SSTable.COMPONENT_DATA, sections,
                    OperationType.BULK_LOAD);
            StreamHeader header = new StreamHeader(directory.getName(), UUID.randomUUID(), pending,
                    Collections.singleton(pending));
